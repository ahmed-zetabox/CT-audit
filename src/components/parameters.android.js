...
export default Parameters = (props) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
   let res
      if (characteristics.length > 0 && isLoading == false && Maintenance_Notify && coldwayIsLoding && props.selectedIndex==0) {
    ...
    }, [characteristics, isLoading,Maintenance_Notify,coldwayIsLoding,props.selectedIndex]),
  );
